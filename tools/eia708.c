...
static int cb_eia_708(void *callback_context, struct vanc_context_s *ctx, struct packet_eia_708b_s *pkt)
{
	int ret = -1;
	...
	uint16_t *words;
	uint16_t wordCount;
	ret = convert_EIA_708B_to_words(pkt, &words, &wordCount);
	if (ret != 0) {
...
static int test_eia_708(struct vanc_context_s *ctx, const uint8_t *buf, size_t bufSize)
{
